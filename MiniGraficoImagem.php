...
{	
	...
	/**
	 * Gera o gr�fico com base no que foi cadastrado anteriormente
	 * 
     ...
     * @param bool $porcentagemNaBarra = true. Indica se colocar� a porcentagem na barra
     * 
     * @throws \UnexpectedValueException Se $this->borda['tipo'] não tiver um valor de 0 a 2
     * @throws \UnexpectedValueException Se a linha de referência ($this->linhaReferencia['valor']) n�o tiver um valor de 1 a 5
     * 
     * @returns @null
	 */  
	public function gerarGrafico($fundovermelho=215, $fundoverde=215, $fundoazul=255, $porcentagemNaBarra=1)
	{
	    if ($this->soma==0) $this->soma="1" ; // PARA EVITAR O ERRO DE DIVISÃO POR 0 - NÃO INFLUENCIA NOS RESULTADOS
	    ...
	    if ($this->ValidaCor($fundovermelho,$fundoverde,$fundoazul)) { // Setando a cor de fundo
		    ...
		    $fundo=\imageColorAllocate($imagem,215,215,215) ; // Configurando a cor de fundo com um cinza padr�o
        }
	    ...
	    for ($cont=0; $cont < $this->indice; $cont++) {
		    ...
		    if (
		        ...
		        throw new \InvalidArgumentException("A(s) cor(es) de uma barra estão com valor(es) inv�lido(s). Devem ser de 0 a 255");
	        }
		    ...
		    if ($this->borda["tipo"]==0 ) {
			    ...
			    throw new \UnexpectedValueException("A borda n�o tinha um valor de 1 a 2");	
			}
		  
		    if ($this->dados[$cont] > 0){	
			    \imageFilledRectangle($imagem, 10, $posicaoY, $posicaoX , $posicaoY + 20, $cor) ; // Colocando a �rea preenchida
		    }

		    \imageString($imagem,5,15,$posicaoY - 17, $this->rotulo[$cont]  ,$corFonte) ; // Colocando a �rea preenchida
	    ...
	    if ($this->linhaReferencia["valor"] < 6 && $this->linhaReferencia["valor"] > 0) {
			...
			throw new \UnexpectedValueException('A linha de refer�ncia ($this->linhaReferencia["valor"]) n�o tinha um valor de 1 a 5');
		}
